...
\section{Sumsetowe pokrycia o minimalnej mocy}
	\subsection{Definicja}
		\begin{frame}{Podstawowe definicje}
			...
			\pause
            \begin{alertblock}{Pokrycie [2]-sumsetowe o minimalnej mocy (\rmfamily \textsc{Minimum cardinality [2]-sumset cover problem})}
                Mając zbiór $ S \subset \N^{+} $, chcemy znaleźć \emph{pokrycie [2]-sumsetowe $ X \subset \N^{+} $ o minimalnej mocy} takie, że $ \forall s \in S, s \in X $ lub $ \exists x_1,x_2 \in X : x_1 + x_2 = s $.
			\end{alertblock}
		\end{frame}
	...
	\subsection{Motywacja}
		\begin{frame}{Do czego to się przydaje}
            \begin{block}{Radioterapia}
    			Uogólniona wersja tego problemu, czyli \emph{Minimum Generating Set} jest związana z radioterapią.
	...
	\subsection{NP-zupełność}
		\begin{frame}{Wcale nie takie proste}
            ...
            \end{block}
            \begin{block}{Generating Set}
                Mając podany zbiór $ S \subset \N^{+} $ oraz $ k > 0 $ sprawdzić czy istnieje zbiór $ T \subset \N^{+} $ taki, że $ |T| = k $ oraz $ \forall s \in S, s $ jest sumą jakiegoś podzbioru elementów T
            \end{block}
		\end{frame}
