	...
	\subsection{APX-trudność}
		\begin{frame}{Aproksymacja też słabo...}
			...
			\begin{exampleblock}{Fakt (też z Wikipedii)}
				Jeśli dla pewnego problemu APX-trudnego istnieje wielomianowy algorytm rozwiązujący go z dowolnie małym błędem multiplikatywnym, to \alt<4->{$ P = NP $.}{...}
			\end{exampleblock}
...
\section{Rozwiązanie}
	...
	\subsection{Algorytm FPT dla przypadku ogólnego}
		\begin{frame}{Algorytmy FPT}
			\begin{block}{Definicja}			
				Niech n będzie rozmiarem danych pewnego problemu, a k jednym z parametrów. Algorytm rozwiązujący ten problem w czasie $ O(f(k)poly(n)) $ dla pewnej funkcji k jest \emph{FPT (Fixed parameter tractable)}.
			\end{block}
